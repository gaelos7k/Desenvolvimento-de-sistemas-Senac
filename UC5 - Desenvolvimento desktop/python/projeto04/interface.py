...
class LibraryApp(ctk.CTk):
    ...
    def show_main_menu(self):
        ...
        for i, data in enumerate(card_data):
            card = ctk.CTkFrame(main_frame, fg_color="#ffffff", width=220, height=320,
                                corner_radius=15, border_width=3, border_color="#e0e0e0")
            card.grid(row=0, column=i, padx=20, pady=25)
            ...
            btn = ctk.CTkButton(card, text=data["button"], fg_color="#b39ddb",
                                ...
                                width=160, height=40, command=data["command"])
            btn.pack(pady=(0, 15))
    ...
    def show_leitor_menu(self):
        ...
        for opcao in opcoes:
            btn = ctk.CTkButton(main_frame, text=opcao["text"], fg_color="#b39ddb",
                                ...
                                width=250, height=35, command=opcao["command"])
            btn.pack(pady=(0, 10))
    ...
    def registrar_emprestimo(self):
        ...
        def registrar():
            ...
            if leitor and livro:
                ...
                CTkMessagebox(title="Sucesso",
                              message=f"Empréstimo registrado!\nLivro: {livro.titulo}\nLeitor: {leitor.nome}")
                dialog.after(100, dialog.destroy)
            else:
                CTkMessagebox(
                    title="Erro", message="Leitor ou livro não encontrado!")
